...
class SettingBloc extends Bloc<SettingEvent, SettingState> {
  ...
  Future<void> _onEvent(SettingEvent event, Emitter<SettingState> emit) async {
    ...
    if (event is ProfileInitial) {
      emit(
          state.copyWith(nShopName: user!.nUserName, nContactNo: user.nUserID));
      ...
      try {
        final Response? response = await nSettingRepository.getData(
            "GEN_UserSP",
            ["@nType", "@nsType", "@UserId", "@OldPassword", "@NewPassword"],
            ['0', '20', user!.nUserID, state.nOldPassword, state.nNewPassword]);
        ...
        if (response != null && response.statusCode == 200) {
          ...
          if (jsonMsg == "Password Changed Successfully") {
            ...
            Fluttertoast.showToast(
                ...
                fontSize: 16.0);
            emit(state.copyWith(
                formStatus: SubmissionFailed(new Exception(jsonMsg))));
          }
        } else {
          Fluttertoast.showToast(
          ...
          emit(state.copyWith(
              formStatus: SubmissionFailed(
                  new Exception("Password failed to change"))));
        }
      } catch (e) {
      ...
      try {
        final Response? response = await nSettingRepository.getData(
            "GEN_BSProOMSSP",
            ["@nType", "@nsType", "@UserId", "@UserName", "@UserCode"],
            ['0', '9', state.nContactNo, state.nShopName, user!.nUserCode]);
  ...
  Future<bool> profileUpdate() async {
    ...
    try {
      ...
      if (response != null && response.statusCode == 200) {
        ...
        emit(state.copyWith(
            formStatus:
                SubmissionFailed(new Exception("Password failed to change"))));
        return false;
      }
    } catch (e) {
      emit(state.copyWith(formStatus: SubmissionFailed(new Exception(e))));
