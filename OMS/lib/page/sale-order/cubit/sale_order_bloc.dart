  ...
  Future<void> _onEvent(
    ...
    if (event is RefreshSaleOrder) {
      emit(state.copyWith(
          ...
          customerState: CustomerState(customerName: '', customerCode: ''),
          salesManTab:
              SalesManTab(nSalesManCode: '', nSalesMan: '', nShortName: ''),
          list: [],
          customerName: '',
          netAmount: 0,
          totalItems: 0,
      ...
      if (event.list.length > 0) {
        ...
        emit(state
            .copyWith(list: [], totalItems: 0, totalQuantity: 0, netAmount: 0));
      }
      ...
      try {
        final Response? response = await saleOrderRepository
            .getSaleOrderList(event.orderType.toString());
        if (response != null) {
          if (response.statusCode == 200) {
            ...
            emit(state.copyWith(
                nOrderList: nlist, formStatus: SubmissionSuccess()));
          }
        }
      } catch (e) {
        emit(state.copyWith(
            formStatus: SubmissionFailed(new Exception("Fetching Failed"))));
      }
      ...
      try {
        ...
        final Response? response = await saleOrderRepository.getData(
            "OMS_SalesOrderSP",
            ["@nType", "@nsType", "@BillNo"],
            ["0", "2", orderId]);
        emit(state.copyWith(
          ...
          if (response.statusCode == 200) {
            ...
            for (var obj in json) {
              Product p = new Product(
                  nBarCode: obj["BarCode"],
                  nItem: obj["Item"],
                  nUnitCode: obj["AttributeCode"] ?? "",
                  nAttribute: obj["Attribute"] ?? "",
                  nAttribute1Image: "",
                  nSalesRate: obj["Rate"].toDouble(),
                  nQuantity: obj["Quantity"].toDouble(),
                  nAmount: obj["Amount"].toDouble());
              list.add(p);
            }
      ...
      if (response != null) {
        if (response.statusCode == 200) {
          EasyLoading.showSuccess(
              state.isEdit == 0 ? "Saved Successfully" : "Update Successfully",
              duration: const Duration(milliseconds: 1000));
          return true;
        }
      }
