...
class ProductLookupState extends State<ProductLookup>
              ...
              child: Container(
                  ...
                  child:  Column(
                    children: <Widget>[
                        ...
                        child: TextField(
                          ...
                          decoration: InputDecoration(
                              ...
                              suffixIcon: editingController.text.length > 1
                                  ? IconButton(
                                  icon: Icon(Icons.close, color: MyConstants.of(context)!.primaryColor),
                                  onPressed: () {
                                    editingController.clear();
                                    FocusManager.instance.primaryFocus
                                        ?.unfocus();
                                    callCustomerOffline(false, editingController.text);
                                  })
                                  : null,
                              ...
                              border: OutlineInputBorder(
                                borderSide: BorderSide(color: MyConstants.of(context)!.primaryColor),
                                  borderRadius:
                                ...
                                children:  ids.map((e) =>
                                    Transform(
                                      ...
                                      child: FilterChip(
                                          ...
                                          avatar: CircleAvatar(
                                              radius: 25,
                                              child: Container(child: e.length > 1 ? Image.memory(base64Decode(e), width: 500, height: 500,gaplessPlayback: true,) : Text(''))
                                          ),
                                          label: Text(''),
                      ...
                      isLoading ? CircularProgressIndicator(color: MyConstants.of(context)!.primaryColor) :
                      Expanded(
                          flex: 20,
  ...
  Widget listViewUI(Product obj, int index) => Card(
    child: Ink(
      color: _SelectedIndex.indexOf(index) >= 0 ? MyConstants.of(context)!.primaryColor.withOpacity(0.3) : Colors.transparent,
      child: ListTile(
        onTap: (){
          setState(() {
            _SelectedIndex.indexOf(index) == -1 ? _SelectedIndex.add(index) : _SelectedIndex.remove(index);
          });
        },
        leading: CircleAvatar(
          child: Container(child: obj.nAttribute1Image.length > 1 ? Image.memory(base64Decode(obj.nAttribute1Image), gaplessPlayback: true,) : Text(obj.nAttribute.substring(0,2)))
        ),
  ...
  Future<void> callCustomerOffline(bool apiCall, String query) async {
    try{
      ...
      if (query.length > 0 && _filters.length < 1) {
        this.list = this.originalList.where((element) => element.nAttribute.contains(query)).toList();
      }
      else if(query.length > 0 && _filters.length > 0){
        this.list = this.originalList.where((element) => element.nAttribute.toLowerCase().contains(query.toLowerCase()) && _filters.contains(element.nAttribute1Image)).toList();
      }
      else if(query.length < 1 && _filters.length > 0){
        this.list = this.originalList.where((element) => _filters.contains(element.nAttribute1Image)).toList();
      }
      else if(apiCall){
        final Response? response = await SaleOrderRepository().getProductLookup();
        if (response != null && response.statusCode == 200) {
          ...
          setState(() { this.list = list; this.originalList = list; List<Product> xList = list.toList(); xList.retainWhere((x) => ids.add(x.nAttribute1Image)); });
        }
      }
      else{
